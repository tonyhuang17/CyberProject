...
1. **Secrets stored in plaintext** in MongoDB
2. **JWT tokens in localStorage** (XSS vulnerable)
3. **No HTTPS/TLS** (localhost HTTP only)
4. **No encryption at rest** for sensitive data

### 🟡 HIGH Vulnerabilities
1. **Weak RBAC implementation** (role not enforced on all endpoints)
2. **No rate limiting** on authentication endpoints
3. **No JWT expiration/refresh** mechanism
4. **No MFA** for privileged accounts

### 🟢 MEDIUM Vulnerabilities
1. **Default test passwords** hardcoded in UI
2. **No session revocation** capability
3. **Browser clipboard access** for secrets (copy function)
4. **No audit logging** for secret access
...
|-------|------------|------------|--------------|
| External Attacker | Data theft, credential harvesting | Medium-High | Network access |
| Malicious Insider | Data exfiltration, sabotage | High | Valid credentials |
| Rogue Developer | Privilege escalation | Medium | Developer account |
| Compromised Account | Automated attacks | Low-Medium | Stolen credentials |